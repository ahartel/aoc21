...
fn traverse_graph_all_small_caves_once(
...
fn traverse_graph_one_small_cave_twice(
    ...
    if is_uppercase || (!small_cave_twice && !is_start_end) || !path.contains(&cur_node.name) {
        let visited_small_twice =
            small_cave_twice || (!is_uppercase && !is_start_end && path.contains(&cur_node.name));
        path.push_str(&cur_node.name);
        path.push_str(",");
        ...
        for child in &cur_node.children {
            if child != "end" {
                let child_node = graph.get(child).unwrap();
                traverse_graph_one_small_cave_twice(
                    graph,
                    child_node,
                    path.clone(),
                    paths,
