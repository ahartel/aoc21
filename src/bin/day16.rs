    ...
    loop {
        match state {
            ...
            ParseState::LengthOrNum => {
                ...
                cur_packet.length_or_num =
                    usize::from_str_radix(&s[idx..idx + field_size], 2).unwrap();
                idx += field_size;
...
fn print_tree(node: &Packet) {
    if node.is_operator {
        if node.is_length_not_num {
            println!(
                " {: <0$}Type: {}, Start Index: {}, Childrenlen: {}, #Children: {}",
                node.depth,
                node.typ,
                node.start_idx,
                node.length_or_num,
            ...
            println!(
                " {: <0$}Type: {}, Start Index: {}, Numchildren: {}, #Children: {}",
                node.depth,
                node.typ,
                node.start_idx,
                node.length_or_num,
        ...
        println!(
            " {: <0$}Type: {}, Value: {}, Start Index: {}",
            node.depth,
            node.typ,
            usize::from_str_radix(&node.literal_value, 2).unwrap(),
            node.start_idx
        );
    }
